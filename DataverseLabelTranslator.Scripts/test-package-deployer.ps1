...
$packageSource = Join-Path $root "DataverseLabelTranslator.Release\$version\appsource\src\DataverseLabelTranslatorPackage"

if ($BuildRelease -or -not (Test-Path -LiteralPath $packageSource)) {
    $releaseScript = Join-Path $root "DataverseLabelTranslator.Scripts\release-appsource.ps1"
