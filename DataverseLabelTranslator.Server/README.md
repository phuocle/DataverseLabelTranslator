...
Copy the repository root `.env.example` to `.env` and fill local DevKit connection values before running the deployment scripts. The `.bat` files are commit-safe and use DevKit `DEVKIT_*` environment variable fallback. Values in `.env` override inherited `DEVKIT_*` environment variables for that batch run.
