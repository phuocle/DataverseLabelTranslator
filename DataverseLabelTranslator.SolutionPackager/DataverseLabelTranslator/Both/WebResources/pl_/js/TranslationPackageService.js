(function (TranslationPackageService, undefined) {
    ...
    function getElements(parent, localName) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            if (
                nodes[i].localName === localName ||
                nodes[i].nodeName === localName ||
                nodes[i].nodeName === "ss:" + localName
            ) {
                result.push(nodes[i]);
            }
        }
    ...
    function getDirectChildren(parent, localName) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (
                child.nodeType === 1 &&
                (child.localName === localName || child.nodeName === localName || child.nodeName === "ss:" + localName)
            ) {
                result.push(child);
            }
        }
    ...
    function exportTranslations(solutionUniqueName) {
        ...
        return WebApiClient.SendRequest("POST", getApiUrl() + "solutions/Microsoft.Dynamics.CRM.ExportTranslation", {
    ...
    function importTranslations(translationFileBase64, importJobId) {
        ...
        return WebApiClient.SendRequest("POST", getApiUrl() + "ImportTranslation()", {
