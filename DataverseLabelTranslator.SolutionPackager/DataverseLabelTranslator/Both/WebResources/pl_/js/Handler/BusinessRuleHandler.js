(function (BusinessRuleHandler, undefined) {
            ...
            .then(function (response) {
                ...
                for (var i = 0; i < attributes.length; i++) {
                    ...
                    attributeDisplayNames[logicalName] =
                        getLocalizedLabel(attribute.DisplayName) || attribute.SchemaName || attribute.LogicalName;
                }
    ...
    function upsertStepLabel(xaml, labelId, languageCode, description) {
        ...
        var existing = findStepLabelTag(xaml, function (attributes) {
            return (
                String(attributes.LabelId).toLowerCase() === String(labelId).toLowerCase() &&
        ...
        return loadAttributeDisplayNames()
            .then(function () {
                return WebApiClient.Retrieve({
                    entityName: "workflow",
                    queryParams:
                        "?$select=workflowid,name,primaryentity,category,statecode,statuscode,scope,clientdata,xaml" +
                        "&$filter=category eq 2 and primaryentity eq '" +
                        escapeODataString(String(entityName || "").toLowerCase()) +
                        "'" +
                        "&$orderby=name asc",
                    returnAllPages: true
                });
        ...
        return WebApiClient.Promise.resolve(workflowIds).each(function (workflowId) {
            ...
            return WebApiClient.Retrieve({
                ...
                .then(function (workflow) {
                    ...
                    if (!originalXaml) {
                        throw new Error(
                            "Business rule " + (workflow.name || workflowId) + " does not contain workflow XAML."
                        );
                    }

                    if (wasActive) {
                        return DeactivateWorkflow(workflowId).then(function () {
                            deactivated = true;
                        });
                    }
                    ...
                    return restoreWorkflow(workflowId, deactivated ? originalXaml : null, wasActive && deactivated)
                        .then(function () {
                            throw new Error(
                                "Failed to save Business Rule translations for " +
                                    ((latestWorkflow && latestWorkflow.name) ||
                                        workflowUpdate.workflowName ||
                                        workflowId) +
                        ...
                        .catch(function (rollbackError) {
                            if (
                                ...
                                rollbackError.message.indexOf("Failed to save Business Rule translations") === 0
                            ) {
                                throw rollbackError;
                            }
