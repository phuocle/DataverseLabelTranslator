(function (DataverseDataWebResourceService, undefined) {
    ...
    function findBaseSolutionByName(solutionUniqueName) {
        return WebApiClient.Retrieve({
            entityName: "solution",
            queryParams:
                "?$select=solutionid,uniquename,friendlyname,_publisherid_value&$filter=uniquename eq '" +
                escapeODataString(solutionUniqueName) +
                "'"
    ...
    function findBaseSolution() {
        return findBaseSolutionByName(BASE_SOLUTION_UNIQUE_NAME).then(function (solution) {
            if (!solution) {
    ...
    function findPublisherByUniqueName(uniqueName) {
        return WebApiClient.Retrieve({
            ...
            queryParams:
                "?$select=publisherid,uniquename,friendlyname,customizationprefix&$filter=uniquename eq '" +
                escapeODataString(uniqueName) +
                "'"
        }).then(function (response) {
            return response && response.value && response.value.length > 0 ? response.value[0] : null;
        });
    ...
    function ensureDataPublisher(basePublisher) {
        ...
        return findPublisherByUniqueName(dataPublisherUniqueName).then(function (existingPublisher) {
    ...
    function findDataSolution() {
        return WebApiClient.Retrieve({
            entityName: "solution",
            queryParams:
                "?$select=solutionid,uniquename,friendlyname,_publisherid_value,version&$filter=uniquename eq '" +
                DATA_SOLUTION_UNIQUE_NAME +
                "'"
        }).then(function (response) {
            return response && response.value && response.value.length > 0 ? response.value[0] : null;
        });
    ...
    function ensureDataSolutionRecord(publisherId) {
        return findDataSolution().then(function (solution) {
            if (solution) {
                return solution;
            }
    ...
    function findWebResourceByName(webResourceName) {
        return WebApiClient.Retrieve({
            overriddenSetName: "webresourceset",
            queryParams:
                "?$select=webresourceid,name,displayname,content,modifiedon,webresourcetype&$filter=name eq '" +
                escapeODataString(webResourceName) +
                "'"
        }).then(function (response) {
            return response && response.value && response.value.length > 0 ? response.value[0] : null;
        });
    ...
    DataverseDataWebResourceService.ReadText = function (options) {
        ...
        return ensureTextWebResource(normalizedOptions).then(function (info) {
            return findWebResourceByName(normalizedOptions.uniqueName).then(function (webResource) {
                if (!webResource) {
    ...
    DataverseDataWebResourceService.ReadJson = function (options) {
        return DataverseDataWebResourceService.ReadText(options).then(function (content) {
            if (!content) {
                return {};
            }
