...
function Resolve-FinalZip {
    ...
    if (-not [string]::IsNullOrWhiteSpace($RequestedVersion)) {
        ...
        $zipPath = Join-Path $Root "release\$RequestedVersion\appsource\zip\DataverseLabelTranslator.v.$packageVersion.zip"
    ...
    if ($candidates.Count -eq 0) {
        throw "No final AppSource ZIP was found under release\<version>\appsource\zip. Run Release AppSource first."
    }
