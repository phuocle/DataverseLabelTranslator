...
Implement Save for `15. Ribbons` using the existing browser-only architecture: edit the loaded helper solution ZIP/XML in memory, import it back to Dataverse, call `PublishAllXmlAsync`, store the returned publish job id in browser `localStorage`, then show a clear message that publish is still running. No server-side C#, Custom API, Custom Action, or MCP ribbon mutation will be added.
...
- When Save is for ribbons, show a confirmation message before importing:
  - tell the user the app will download the current entity ribbon backup
  - show the exact backup filename
  - `Yes` downloads the prepared backup immediately and continues Save
  - `No` cancels Save and leaves the grid unchanged
- Block Save if backup preparation or download fails; never import ribbon XML without a confirmed backup download.
- Treat `PublishAllXmlAsync` as fire-and-track: do not block Save waiting for publish completion and do not immediately reload the grid.
- After `PublishAllXmlAsync` returns the job id, show a persistent Publish XML banner and a message telling the user that Publish XML is running. The grid and toolbar are unlocked after the message closes; subsequent Save/Load actions check the stored job and are blocked only while the job is still non-terminal.

## Public Interfaces

- `RibbonHandler.Save()` becomes functional.
- `XrmTranslator` gets a global Save guard that runs before every type-specific handler Save, not only before ribbon Save.
...
- Every click on the global Save button must check the stored publish job before it calls the current handler's `Save()`.
...
- If Dataverse returns a non-terminal job record, show an error and stop Save before any handler writes data.
- If Dataverse returns a terminal job record or not found, clear the localStorage key and continue Save normally.
- Each blocked Save/Load check increments `blockedAttempts`; at 10 blocked checks, clear the local guard and let the user continue.
...
- The user-facing post-save message and banner must say that Publish XML is running and the user must wait a few minutes before loading, saving, or updating ribbon customizations again. The UI must not remain globally readonly after the message is dismissed.
...
- If the user clicks `Yes`, the app downloads the prepared backup immediately and only then continues to `ImportSolution`.
- If the user clicks `No`, Save is cancelled and no Dataverse write happens.
- The user must keep this downloaded backup before continuing. If the browser blocks the download or backup generation throws, Save stops before `ImportSolution`.
...
- Manual happy path: edit Japanese/Vietnamese ribbon Text/Title/Description, Save, confirm the backup dialog shows the exact filename, click `Yes`, confirm the backup downloads, confirm the app shows the publish-running message, wait for publish to finish, then reload and confirm values persist.
- Async publish path: after ribbon Save, confirm `PublishAllXmlAsync` returns a job id and the app stores it in `localStorage`.
- Guard block path: with a stored job id that still exists on the server, click Save for any type and confirm Save is blocked before the handler writes anything.
- Guard stale path: with a stored job id that no longer exists on the server, click Save and confirm the app clears localStorage and continues Save.
- Cancel path: edit a ribbon label, click Save, click `No` in the backup dialog, confirm no backup download, no import, and pending grid changes remain.
...
- Save waits for `ImportSolution` to finish, then calls `PublishAllXmlAsync` and stores the returned job id; it does not wait for Dataverse publish completion.
...
- User must wait a few minutes for Dataverse Publish XML to complete before saving again. Any Save while the stored publish job still exists on the server is blocked.
