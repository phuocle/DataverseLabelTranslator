(function (RibbonHandler, undefined) {
    ...
    function exportSelectedSolution(solution) {
        ...
        return WebApiClient.Execute(
            WebApiClient.Requests.ExportSolutionRequest.with({
                payload: {
    ...
    function getAttributeInsensitive(node, name) {
        return (
            node.getAttribute(name) || node.getAttribute(name.toLowerCase()) || node.getAttribute(name.toUpperCase())
        );
    }
    ...
    function isControlNode(node) {
        return (
            node && (node.nodeName === "Button" || node.nodeName === "SplitButton" || node.nodeName === "FlyoutAnchor")
        );
    }
    ...
    function appendControlRecords(records, seen, controlNode, entityInfo, locLabels, controlIndex) {
        ...
        if (children.length > 0) {
            records.push({
                ...
                schemaName: getControlNodeText(
                    surfaceLabel,
                    controlNode,
                    getControlPreviewLabel(controlNode, locLabels)
                ),
                entityLogicalName: entityInfo.logicalName,
                component: controlNode.nodeName,
                surface: surface,
    ...
    function readCustomizationsFromExport(exportResponse) {
        ...
        return JSZip.loadAsync(exportResponse.ExportSolutionFile, { base64: true }).then(function (zip) {
            var entry = zip.file("customizations.xml");
            if (!entry) {
                throw new Error("Exported solution does not contain customizations.xml.");
            }

            return entry.async("string").then(function (customizationsXml) {
                return {
                    zip: zip,
    ...
    function prepareBackupArtifact(changes) {
        ...
        backupZip.file(ribbonState.solutionUniqueName + "-original.zip", ribbonState.solutionZipBase64, {
            base64: true
        });
        ...
        return backupZip.generateAsync({ type: "blob" }).then(function (blob) {
            return {
                fileName: fileName,
                blob: blob
            };
    ...
    function createImportSolutionBase64(updatedCustomizationsXml) {
        return JSZip.loadAsync(ribbonState.solutionZipBase64, { base64: true }).then(function (zip) {
            zip.file("customizations.xml", updatedCustomizationsXml);
            return zip.generateAsync({ type: "base64" }).then(function (importZipBase64) {
                return {
                    zip: zip,
    ...
    function importRibbonSolution(importZipBase64) {
        ...
        return XrmTranslator.RunAsBaseLanguage(function () {
            return WebApiClient.SendRequest(
                "POST",
                WebApiClient.GetApiUrl({ apiVersion: "9.2" }) + "ImportSolution()",
                {
    ...
    RibbonHandler.Load = function () {
        ...
        return getSelectedSolution()
            .then(function (solution) {
                return exportSelectedSolution(solution).then(function (exportResponse) {
                    return {
                        solution: solution,
            ...
            .then(function (context) {
                return readCustomizationsFromExport(context.exportResponse).then(function (exportData) {
            ...
            .then(function (exportData) {
                ...
                if (!entityNode) {
                    throw new Error(
                        "Entity " + entityInfo.logicalName + " was not found in exported customizations.xml."
                    );
                }
