(function (ModernCommandHandler, undefined) {
    ...
    function retrieveAll(url) {
        ...
        function next(nextUrl) {
            return WebApiClient.SendRequest("GET", nextUrl).then(function (response) {
    ...
    function getLocalizedLabel(labelCollection) {
        var labels =
            labelCollection && labelCollection.Label && labelCollection.Label.LocalizedLabels
    ...
    function retrieveLocLabels(commandId, propertyName) {
        ...
        var url =
            getApiUrl() +
            "RetrieveLocLabels(EntityMoniker=@p1,AttributeName='" +
            propertyName +
    ...
    function retrieveCommandLabels(command) {
        ...
        for (var i = 0; i < translatableAttributes.length; i++) {
            (function (propertyName) {
                requests.push(
                    retrieveLocLabels(command.appactionid, propertyName).then(function (labels) {
        ...
        return WebApiClient.Promise.all(requests).then(function () {
            return command;
        });
    }
    ...
    function getSolutionAppActionIds() {
        ...
        return retrieveAll(
            getApiUrl() +
                "solutioncomponents?$select=objectid,componenttype&$filter=_solutionid_value eq " +
                solutionId +
                " and componenttype eq " +
                APP_ACTION_COMPONENT_TYPE
    ...
    function FillTable() {
        ...
        for (var c = 0; c < commandData.length; c++) {
            ...
            for (var p = 0; p < translatableAttributes.length; p++) {
                ...
                var labelCollection =
                    command.labels && command.labels[property.propertyName]
                ...
                var labels =
                    labelCollection.Label && labelCollection.Label.LocalizedLabels
                        ? labelCollection.Label.LocalizedLabels
                        : [];
