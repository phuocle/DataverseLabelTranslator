(function (EntityMessageHandler, undefined) {
    ...
    function getSelectedEntityInfo() {
        ...
        return WebApiClient.Retrieve({
            ...
            queryParams:
                "?$select=LogicalName,ObjectTypeCode,SchemaName,DisplayName,DisplayCollectionName,IsCustomEntity"
    ...
    function findDisplayStringsWorksheet(workbook) {
        ...
        for (var j = 0; j < workbook.worksheets.length; j++) {
            if (
                normalize(workbook.worksheets[j].name).indexOf("display") !== -1 &&
    ...
    function findHeaderRow(worksheet) {
        ...
        for (var r = 0; r < rows.length; r++) {
            ...
            if (
                (texts.indexOf("display") !== -1 || texts.indexOf("resource") !== -1 || texts.indexOf("key") !== -1) &&
    ...
    function applyChangesToPackage(packageData, identitySet, entityInfo, changes) {
        ...
        for (var i = 0; i < changes.length; i++) {
            ...
            if (column == null) {
                throw new Error(
                    "Language column " + change.language + " was not found in the fresh translation package."
                );
            }
    ...
    EntityMessageHandler.Load = function () {
        ...
        return Promise.all([getSolutionUniqueName(XrmTranslator.GetSolution()), getSelectedEntityInfo()])
