(function (SiteMapHandler, undefined) {
    ...
    function GetNodeDisplayValue(node) {
        return (
            GetCleanText(node.getAttribute("Title")) ||
    ...
    function GetSiteMapDisplayName(sitemap) {
        return GetCleanText(sitemap.sitemapname) || GetSiteMapLogicalName(sitemap) || "Unnamed SiteMap";
    }
    ...
    SiteMapHandler.Load = function () {
        ...
        if (solutionId && solutionId !== "all") {
            ...
            sitemapPromise = WebApiClient.Retrieve({
                ...
                var componentIds = (response.value || []).map(function (c) {
                    return c.objectid;
                });
                ...
                var filterParts = componentIds.map(function (id) {
                    return "sitemapid eq " + id;
                });
                return WebApiClient.Retrieve({
                    entityName: "sitemap",
            ...
            .then(function (response) {
                ...
                if (sitemaps.length === 0) {
                    ...
                    return DialogHelper.alert(
                        "No sitemaps found" + (solutionId && solutionId !== "all" ? " in the selected solution." : ".")
                    );
                }
                ...
                for (var s = 0; s < updatedSiteMapIds.length; s++) {
                    (function (siteMapId, index) {
                        saveChain = saveChain.then(function () {
                            ...
                            XrmTranslator.LockGridProgress(
                                "Saving " + XrmTranslator.GetCurrentToolbarTypeText(),
                                index + 1,
                                updatedSiteMapIds.length
                            );
