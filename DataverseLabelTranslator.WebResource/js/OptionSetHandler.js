(function (OptionSetHandler, undefined) {
    ...
    OptionSetHandler.Load = function () {
        ...
        var optionSetRequest = {
            ...
            queryParams:
                "/Attributes/Microsoft.Dynamics.CRM.PicklistAttributeMetadata?$expand=OptionSet,GlobalOptionSet"
        };
        ...
        var multiOptionSetRequest = {
            ...
            entityId: entityMetadataId,
            queryParams:
                "/Attributes/Microsoft.Dynamics.CRM.MultiSelectPicklistAttributeMetadata?$expand=OptionSet,GlobalOptionSet"
        };

        return WebApiClient.Promise.all([
            WebApiClient.Retrieve(optionSetRequest),
            WebApiClient.Retrieve(booleanRequest),
            WebApiClient.Retrieve(statusRequest),
            WebApiClient.Retrieve(stateRequest),
            WebApiClient.Retrieve(multiOptionSetRequest)
        ])
            .then(function (responses) {
                var responseValues = responses[0].value
                    .concat(responses[1].value)
                    .concat(responses[2].value)
                    .concat(responses[3].value)
                    .concat(responses[4].value);
    ...
    OptionSetHandler.SaveOnly = function () {
        ...
        return SaveOptionValueUpdates(updates).then(function () {
